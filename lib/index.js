...
/**
 * Construct a handler object which contains in and out
 * functions.
 * @param  {Array} fns 
 * @return {Object}     
 */

function handlers(fns){
  return {
    in: fns
  }
}
...
transit.define = function define(url, fns){
  transit.regexps.push(toRegexp(url, []));
  var handle = handlers(fns);
  transit.currentHandler = handle;
  transit.handlers.push(handle);
};
...
/**
 * Checks to see if the route is the same as our
 * current route. Used primarily to filter hashchange events
...
transit.exec = function exec(url){
  var i = transit.lookup(url);

  if (url != transit.url) transit.emit('change', url);

  var out = transit.previous && transit.previous.out;
  if (out) middleware(transit.currentContext, out);

  if (typeof i != 'undefined') {
    var ctx = transit.context(url);
    transit.currentContext = ctx;
    var fns = transit.handlers[i].in;
    middleware(ctx, fns);
    transit.previous = transit.handlers[i];
    transit.url = url;
...
transit.lookup = function lookup(url){
  var regexps = transit.regexps;
  for (var i = 0, l = regexps.length; i < l; i++) {
    if (regexps[i].test(url)) return i;
  }
};
